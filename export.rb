...
class Export
  ...
  def all_tables()
    ...
    return @client.query(show_tables)
  end

  def show_full_columns_from(table) 
    show_full_columns = %{show full columns from #{table}}
    return @client.query(show_full_columns)
  end
  ...
  def set_table_define()
    cell_position = 4
    all_tables.each do |res|
      table = res["Tables_in_portweb"]
      results = show_full_columns_from(table)
      @wb.styles do |s|
        grey_cell = s.add_style :bg_color => "e6e6e6", :border => {:style => :thin, :color => "00"}, :alignment => {:horizontal => :center}
        normal_cell = s.add_style :bg_color => false, :border => {:style => :thin, :color => "00"}
        cell = s.add_style :bg_color => false
  ...
  def create()
    set_table_list()
    set_table_define()
    if @package.serialize(ARGV[0] + '.xlsx')
      puts "-----------------------------"
      puts "Success Create " + ARGV[0] + '.xlsx'
      puts "-----------------------------"
    else
      puts "-----------------------------"
      puts "Error."
